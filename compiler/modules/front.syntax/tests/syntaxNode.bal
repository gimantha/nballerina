...
function rootSyntaxNode(ModulePart part) returns RootSyntaxNode {
    SyntaxNode[] importDecls = from ImportDecl decl in part.importDecls
        select syntaxNodeFromImportDecl(decl);
    SyntaxNode[] moduleLevelDefns = from ModuleLevelDefn defn in part.defns
        select syntaxNodeFromModuleLevelDefn(defn);
...
function syntaxNodeFromImportDecl(ImportDecl decl) returns NonTerminalSyntaxNode {
    ...
    SyntaxNode[] nameNodes = flattenSyntaxNodeList(from int i in 0 ..< decl.names.length()
        select
...
function syntaxNodeFromStmtBlock(StmtBlock block) returns NonTerminalSyntaxNode {
    SyntaxNode[] body = from Stmt stmt in block.stmts
        select syntaxNodeFromStmt(stmt);
...
function syntaxNodeFromMatchStmt(MatchStmt stmt) returns NonTerminalSyntaxNode {
    SyntaxNode[] clauses = from MatchClause clause in stmt.clauses
        select syntaxNodeFromMatchClause(clause);
    return nonTerminalSyntaxNode(stmt,
...
function syntaxNodeFromMatchPatterns(MatchPattern[] patterns) returns NonTerminalSyntaxNode[] {
    SyntaxNode[][] childNodes = from int i in 0 ..< patterns.length()
        select i > 0 ? [{token: "|"}, syntaxNodeFromMatchPattern(patterns[i])] : [syntaxNodeFromMatchPattern(patterns[i])];
    return from int i in 0 ..< patterns.length()
        select nonTerminalSyntaxNode(patterns[i], childNodes[i]);
}
...
function syntaxNodeFromFunctionCallExpr(FunctionCallExpr expr) returns NonTerminalSyntaxNode {
    SyntaxNode[] args = flattenSyntaxNodeList(from int i in 0 ..< expr.args.length()
        select fixedTokenSeperatedSyntaxNode(i, syntaxNodeFromExpr(expr.args[i]), ","));
...
function syntaxNodeFromMethodCallExpr(MethodCallExpr expr) returns NonTerminalSyntaxNode {
    SyntaxNode[] args = from Expr arg in expr.args
        select syntaxNodeFromExpr(arg);
...
function syntaxNodeFromListConstructorExpr(ListConstructorExpr expr) returns NonTerminalSyntaxNode {
    SyntaxNode[] members = flattenSyntaxNodeList(from int i in 0 ..< expr.members.length()
        select fixedTokenSeperatedSyntaxNode(i, syntaxNodeFromExpr(expr.members[i]), ","));
    return nonTerminalSyntaxNode(expr, {token: "[", pos: expr.startPos},
                                        // JBUG: can't use the query exprssion directly
                                        members,
                                        {token: "]"});
...
function syntaxNodeFromMappingConstructorExpr(MappingConstructorExpr expr) returns NonTerminalSyntaxNode {
    SyntaxNode[] fields = flattenSyntaxNodeList(from int i in 0 ..< expr.fields.length()
        select fixedTokenSeperatedSyntaxNode(i, syntaxNodeFromField(expr.fields[i]), ","));
    return nonTerminalSyntaxNode(expr, {token: "{", pos: expr.startPos},
                                        // JBUG: can't use the query exprssion directly
                                        fields,
                                        {token: "}"});
...
function syntaxNodeFromTupleTypeDesc(TupleTypeDesc td) returns NonTerminalSyntaxNode {
    SyntaxNode[] memberNodes = flattenSyntaxNodeList(from int i in 0 ..< td.members.length()
        select fixedTokenSeperatedSyntaxNode(i, syntaxNodeFromTypeDesc(td.members[i]), ","));
...
function syntaxNodeFromArrayTypeDesc(ArrayTypeDesc td) returns NonTerminalSyntaxNode {
    SyntaxNode[][] dimensions = from SimpleConstExpr? dimension in td.dimensions
        select dimension == () ? [{token: "["}, {token: "]"}] :
...
function syntaxNodeFromMappingTypeDesc(MappingTypeDesc td) returns NonTerminalSyntaxNode {
    ...
    if td.fields.length() > 0 {
        SyntaxNode[] fields = from FieldDesc f in td.fields
            select syntaxNodeFromFieldDesc(f);
...
function syntaxNodeFromFunctionTypeDesc(FunctionTypeDesc td, boolean functionSignature = false) returns SyntaxNode {
    SyntaxNode[] params = flattenSyntaxNodeList(from int i in 0 ..< td.params.length()
        select fixedTokenSeperatedSyntaxNode(i, syntaxNodeFromFunctionTypeParam(td.params[i]), ","));
